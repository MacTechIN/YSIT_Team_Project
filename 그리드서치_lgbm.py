...
# XGBoost 모델 정의
model = LGBMRegressor(categorical_column=categorical_cols)

#탐색할 하이퍼파라미터 그리드 정의
param_grid = {
    'n_estimators': [400],
    'learning_rate': [0.01, 0.02],    
    'subsample': [0.8,0.9,1],  # 각 트리에 사용할 훈련 데이터 샘플 비율
    'colsample_bytree': [0.8,0.9,1],  # 각 트리에 사용할 특성의 비율
...
grid_search = GridSearchCV(model, param_grid, scoring=scorer, cv=2)
...
#%%
joblib.dump(best_model, path + '/ML/bestLGboost_model_D_travel.pkl')
